...
local insert_quarto_block = function(block_type, placeholder)
  vim.api.nvim_feedkeys(vim.api.nvim_replace_termcodes('<esc>', true, false, true), 'n', true)
  
  local keys
  if placeholder then
    -- Insert block with placeholder content
    keys = [[o:::]] .. block_type .. [[<cr>]] .. placeholder .. [[<cr>:::<esc>]]
  else
    -- Insert both tags on consecutive lines without empty line between
    keys = [[o:::]] .. block_type .. [[<cr>:::<esc>k$a]]
  end
...
-- Special direction block with embedded Python code
local insert_direction_block = function()
  local python_code = "```{python}\n#| echo: false\nprint('\\u200b')\n```"
  insert_quarto_block('{.direction}', python_code)
end

-- Other blocks without placeholders
local insert_solution_block = function()
  insert_quarto_block('{.sol}')
end

local insert_exercise_block = function()
  insert_quarto_block('{#exr-}')
end
...
-- Store terminal buffers and window information
local terminal_buffers = {}  -- Store buffers for each terminal type
local terminal_window = nil  -- Store current terminal window

local function toggle_terminal_split()
  -- If terminal window exists and is valid, just close the window
  ...
  -- Get the most recently used terminal buffer
  local recent_buf = nil
  for _, buf in pairs(terminal_buffers) do
  ...
  -- If we have a valid terminal buffer, show it in a new split
  if recent_buf then
    vim.cmd('vsplit')
    terminal_window = vim.api.nvim_get_current_win()
    vim.api.nvim_win_set_buf(terminal_window, recent_buf)
  end
end

-- Modified new_terminal function
local function new_terminal(lang)
  -- If we don't have a terminal for this language yet, create one
  if not terminal_buffers[lang] or not vim.api.nvim_buf_is_valid(terminal_buffers[lang]) then
    vim.cmd('vsplit term://' .. lang)
    terminal_buffers[lang] = vim.api.nvim_get_current_buf()
    terminal_window = vim.api.nvim_get_current_win()
  else
    -- If terminal window doesn't exist, create it
    if not terminal_window or not vim.api.nvim_win_is_valid(terminal_window) then
      vim.cmd('vsplit')
      terminal_window = vim.api.nvim_get_current_win()
      vim.api.nvim_win_set_buf(terminal_window, terminal_buffers[lang])
    end
  end
end

-- Modified specific terminal functions
...
wk.add({
  {
    ...
    { "<leader>ct", toggle_terminal_split, desc = "[t]oggle terminal split" },
    ...
    -- insert different blocks in (quarto) markdown
    { "<leader>i", group = "[i]nsert" },
    { "<leader>ip", insert_py_chunk, desc = "insert [p]ython code chunk" },
    { "<leader>ir", insert_r_chunk, desc = "insert [r] code chunk" },
    { "<leader>ie", insert_exercise_block, desc = "insert [e]xercise block" },
    { "<leader>id", insert_direction_block, desc = "insert [d]irection block" },
    { "<leader>is", insert_solution_block, desc = "insert [s]olution block" },
